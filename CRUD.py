import shelve
archive = "base_students"
loop = True
characters="{}"

def Create_student():
    with shelve.open(archive, writeback=True) as students:
        courses={}
        course2={}
    ...
    with shelve.open(archive, writeback=True) as students:
        ...
        if str(carnet_u) in students:
            ...
            if option_u==1:
                ...
                if course_u in (students[str(carnet_u)]["Cursos"]):
                    for i,j in students.items():
                        for k,l in j['Cursos'].items():
                            if i==str(carnet_u) and k == course_u:
    ...
    with shelve.open(archive, writeback=True) as students:
        ...
        if str(carnet_d) in students:
            ...
            if option_d==2:
                ...
                if course_d in (students[str(carnet_d)]["Cursos"]):
